...
const { addUser, removeUser, getUser, getUserInRoom, setUserSrc,setMessageTime } = require('./users');
...
const io = socketio(server);
    ...
    io.on('connection', ( socket ) => {
        ...
        const time =   setMessageTime();
        
        socket.emit('message',{user:'admin', text:`${user.name}, welcome to the room ${user.room}!`,time});
        socket.broadcast.to(user.room).emit('message',{ user: 'admin', text: `${user.name},has joined!`,time })
